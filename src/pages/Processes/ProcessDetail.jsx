...
export function ProcessDetail() {
  ...
  const [isEditMovDateOpen, setIsEditMovDateOpen] = useState(false);
  const [selectedMovDate, setSelectedMovDate] = useState(null);
  const [newMovDateVal, setNewMovDateVal] = useState('');
  const [movDateLoading, setMovDateLoading] = useState(false);
  const [movDateAlert, setMovDateAlert] = useState('');
  // Form states
  ...
  const currentProjKey = useMemo(() => {
    if (checklistType === 'residencial') return 'projeto_residencial';
    if (checklistType === 'comercial') return 'projeto_comercial';
    if (checklistType === 'loteamento') return 'projeto_loteamento';
    if (checklistType === 'condominio') return 'projeto_condominio';
    return 'projeto_residencial';
  }, [checklistType]);
    ...
    try {
      ...
      for (const key of Object.keys(uploadedFiles)) {
        ...
        if (Array.isArray(fileData)) {
          if (!docsCopy[key]) docsCopy[key] = { status: 'apresentado' };
          if (!docsCopy[key].files) docsCopy[key].files = [];
          
          // Migrate legacy single file if exists
          if (docsCopy[key].url && docsCopy[key].name) {
            docsCopy[key].files.push({ name: docsCopy[key].name, url: docsCopy[key].url });
            docsCopy[key].name = '';
            docsCopy[key].url = '';
          }
          ...
          for (const file of fileData) {
            if (file instanceof File) {
              ...
              docsCopy[key].files.push({ name: file.name, url: publicUrl });
              updatedSomeUrls = true;
            }
          }
          docsCopy[key].status = 'apresentado';
        } else if (fileData && fileData instanceof File) {
          const publicUrl = await api.uploadFile(fileData);
          if (docsCopy[key]) {
            docsCopy[key].url = publicUrl;
            docsCopy[key].name = fileData.name;
            docsCopy[key].status = 'apresentado';
            updatedSomeUrls = true;
          }
        }
      ...
      if (updatedSomeUrls) {
        ...
        for (const key of Object.keys(newUploaded)) {
          if (key !== 'lic_ambient_files') {
             if (Array.isArray(newUploaded[key])) {
                newUploaded[key] = newUploaded[key].filter(f => !(f instanceof File));
                if (newUploaded[key].length === 0) delete newUploaded[key];
             } else if (newUploaded[key] instanceof File) {
  ...
  const extractTextFromDoc = async (docItem, key) => {
    ...
    if (key === 'lic_ambient_files') {
      ...
      if (Array.isArray(uploadedFiles[key])) {
        fileObj = uploadedFiles[key].find(f => f.name === docItem.name);
      } else {
  ...
  const handleRunAuditoria = async () => {
    ...
    try {
      ...
      docKeys.forEach(key => {
        ...
        if (doc && doc.status === 'apresentado') {
          const files = [];
          if (doc.url) files.push({ url: doc.url, name: doc.name || `${key}.pdf` });
          if (doc.files && Array.isArray(doc.files)) {
            files.push(...doc.files);
          }
          if (files.length > 0) {
            docsToSend[key] = files;
          }
        }
      });
...
`;
  ...
  const handleUpdateMovDate = async () => {
    if (!newMovDateVal) { setMovDateAlert('Insira uma data/hora válida.'); return; }
    setMovDateLoading(true);
    setMovDateAlert('');
    try {
      const isoDate = new Date(newMovDateVal).toISOString();
      await api.updateMovementDate(selectedMovDate.id, isoDate, user, proc.protocol, selectedMovDate.label);
      setIsEditMovDateOpen(false);
      loadData();
    } catch(err) {
      setMovDateAlert('Erro: ' + err.message);
    } finally {
            ...
            return (
              <div key={m.id} className="tl-item">
                ...
                <div className="tl-content">
                  ...
                  <div className="fca gap8" style={{fontSize: '11px', color: 'var(--text3)'}}>
                    {fmtDT(m.created_at)}
                    {user?.role === 'admin' && (
                      <button className="btn btn-outline btn-sm" style={{padding: '0px 4px', fontSize: '10px', height: 'auto', minHeight: 0, color: 'var(--blue)', border: 'none', background: 'transparent', cursor: 'pointer'}} title="Alterar Data Retroativa" onClick={() => {
                        setSelectedMovDate(m);
                        setNewMovDateVal(m.created_at ? new Date(new Date(m.created_at).getTime() - (new Date().getTimezoneOffset() * 60000)).toISOString().slice(0, 16) : '');
                        setIsEditMovDateOpen(true);
                      }}>📅</button>
                    )}
                  </div>
...
ON public.process_checklists FOR ALL TO authenticated USING (true) WITH CHECK (true);`}
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              ...
              <button 
                type="button"
                className={`btn ${checklistType === 'condominio' ? 'btn-primary' : 'btn-outline'}`}
                onClick={() => setChecklistType('condominio')}
                style={{ padding: '8px 16px', fontSize: '13px', borderRadius: '20px' }}
              >
                🏢 Condomínio
              </button>
              <button 
                type="button"
                className={`btn ${checklistType === 'loteamento' ? 'btn-primary' : 'btn-outline'}`}
                onClick={() => setChecklistType('loteamento')}
                style={{ padding: '8px 16px', fontSize: '13px', borderRadius: '20px' }}
              >
                📐 Loteamento
              </button>
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {[
                    ...
                    const localFiles = Array.isArray(uploadedFiles[doc.key]) ? uploadedFiles[doc.key] : (uploadedFiles[doc.key] ? [uploadedFiles[doc.key]] : []);
                    const persistedFiles = checklistData.documentos[doc.key]?.files || [];
                    const legacyFile = checklistData.documentos[doc.key]?.url ? [{ name: checklistData.documentos[doc.key].name, url: checklistData.documentos[doc.key].url }] : [];
                    
                    const allFiles = [
                      ...legacyFile,
                      ...persistedFiles,
                      ...localFiles.map(f => ({ name: f.name, url: '', isLocal: true }))
                    ];
                      ...
                      <div key={doc.key} style={{ display: 'grid', gridTemplateColumns: '1fr 230px 220px', gap: '12px', alignItems: 'center', padding: '8px', background: 'var(--body-bg)', borderRadius: '4px', border: '1px solid var(--border)' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          {allFiles.length > 0 && allFiles.map((file, idx) => (
                            <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '6px', background: file.isLocal ? 'rgba(34, 197, 94, 0.08)' : 'rgba(59, 130, 246, 0.08)', padding: '2px 8px', borderRadius: '4px', border: `1px dashed ${file.isLocal ? 'var(--green)' : 'var(--blue)'}`, width: '100%', justifyContent: 'space-between' }}>
                              <span 
                                style={{ fontSize: '10px', color: file.isLocal ? 'var(--green)' : 'var(--blue)', fontWeight: '600', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap', maxWidth: '120px' }} 
                                title={`${file.name} ${file.isLocal ? '(Pronto para salvar)' : '(Salvo)'}`}
                              >
                                📄 {file.name}
                              </span>
                              <div style={{ display: 'flex', gap: '4px' }}>
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    if (file.isLocal) {
                                      setUploadedFiles(prev => {
                                        const newFiles = (prev[doc.key] || []).filter(f => f.name !== file.name);
                                        return { ...prev, [doc.key]: newFiles.length > 0 ? newFiles : null };
                                      });
                                    } else {
                                      const newDocs = { ...checklistData.documentos };
                                      if (newDocs[doc.key].files) {
                                        newDocs[doc.key].files = newDocs[doc.key].files.filter(f => f.url !== file.url);
                                      }
                                      if (newDocs[doc.key].url === file.url) {
                                        newDocs[doc.key].url = '';
                                        newDocs[doc.key].name = '';
                                      }
                                      if ((!newDocs[doc.key].files || newDocs[doc.key].files.length === 0) && !newDocs[doc.key].url) {
                                        newDocs[doc.key].status = 'ausente';
                                      }
                                      setChecklistData({ ...checklistData, documentos: newDocs });
                                    }
                          ...
                          <label className="btn btn-outline btn-sm" style={{ cursor: 'pointer', margin: 0, padding: '2px 8px', fontSize: '10px', height: '24px', whiteSpace: 'nowrap', display: 'inline-flex', alignItems: 'center', gap: '4px', width: 'fit-content' }}>
                            📁 Anexar (Múltiplos)
                            <input 
                              type="file"
                              accept=".pdf,.md,.txt"
                              ...
                              onChange={e => {
                                if (e.target.files && e.target.files.length > 0) {
                                  ...
                                  setUploadedFiles(prev => {
                                    const current = prev[doc.key];
                                    const currentArr = Array.isArray(current) ? current : (current ? [current] : []);
                                    return { ...prev, [doc.key]: [...currentArr, ...filesArray] };
                                  });
                                  const newDocs = { ...checklistData.documentos };
                                  newDocs[doc.key].status = 'apresentado';
          ...
          {checklistType === 'residencial' && (() => {
            ...
            return (
              <div style={{ marginBottom: '28px' }}>
                ...
                <div style={{ padding: '16px', background: 'rgba(0,0,0,0.02)', border: '1px solid var(--border)', borderRadius: '6px', marginBottom: '20px' }}>
                  <strong style={{ display: 'block', fontSize: '13px', marginBottom: '12px', color: 'var(--text2)', borderBottom: '1px solid var(--border)', paddingBottom: '6px' }}>
                ...
                <CardAfastamentos
                  zonaKey={checklistData[currentProjKey]?.zona_kmz || ''}
                  numPavimentos={checklistData[currentProjKey]?.num_pavimentos || ''}
                  nsapl={checklistData[currentProjKey]?.afast_nsapl || false}
                  setNsapl={val => setChecklistData(prev => ({
                    ...prev,
                    [currentProjKey]: { ...prev[currentProjKey], afast_nsapl: val }
                  }))}
                  esquina={checklistData[currentProjKey]?.afast_esquina || 'nao'}
                  setEsquina={val => setChecklistData(prev => ({
                    ...prev,
                    [currentProjKey]: { ...prev[currentProjKey], afast_esquina: val }
                  }))}
                  adotaAlinhamento={checklistData[currentProjKey]?.afast_adota_alinhamento || false}
                  setAdotaAlinhamento={val => setChecklistData(prev => ({
                    ...prev,
                    [currentProjKey]: { ...prev[currentProjKey], afast_adota_alinhamento: val }
                  }))}
                  frontal1={checklistData[currentProjKey]?.afast_frontal1 || ''}
                  setFrontal1={val => setChecklistData(prev => ({
                    ...prev,
                    [currentProjKey]: { ...prev[currentProjKey], afast_frontal1: val }
                  }))}
                  frontal2={checklistData[currentProjKey]?.afast_frontal2 || ''}
                  setFrontal2={val => setChecklistData(prev => ({
                    ...prev,
                    [currentProjKey]: { ...prev[currentProjKey], afast_frontal2: val }
                  }))}
                  lateral={checklistData[currentProjKey]?.afast_lateral || ''}
                  setLateral={val => setChecklistData(prev => ({
                    ...prev,
                    [currentProjKey]: { ...prev[currentProjKey], afast_lateral: val }
                  }))}
                  fundos={checklistData[currentProjKey]?.afast_fundos || ''}
                  setFundos={val => setChecklistData(prev => ({
                    ...prev,
                    [currentProjKey]: { ...prev[currentProjKey], afast_fundos: val }
                  }))}
                  numeroItem={11}
                />
          ...
          {/* Seção 4: Análise do Projeto Arquitetônico (Condomínio e Loteamento) */}
          {(checklistType === 'condominio' || checklistType === 'loteamento') && (
            <div style={{ marginBottom: '28px' }}>
              <h3 style={{ fontSize: '15px', fontWeight: '600', marginBottom: '12px', color: 'var(--text1)' }}>
                📐 4. Análise do Projeto Arquitetônico ({checklistType === 'condominio' ? 'Condomínio' : 'Loteamento'})
              </h3>

              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '20px' }}>

                {/* Info de desenvolvimento */}
                <div style={{ padding: '20px', background: 'rgba(59, 130, 246, 0.05)', border: '1px solid rgba(59, 130, 246, 0.2)', borderRadius: '8px', gridColumn: '1 / -1' }}>
                  <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                    <span style={{ fontSize: '28px', lineHeight: '1' }}>🏗️</span>
                    <div>
                      <strong style={{ display: 'block', fontSize: '14px', color: 'var(--blue)', marginBottom: '6px' }}>
                        Checklist Técnico — Modalidade: {checklistType === 'condominio' ? 'Condomínio' : 'Loteamento'}
                      </strong>
                      <p style={{ margin: '0 0 8px 0', fontSize: '12px', color: 'var(--text2)', lineHeight: '1.5' }}>
                        O checklist detalhado específico para esta modalidade está em desenvolvimento.
                        Os dados de <strong>Análise de Afastamentos (item 14)</strong> já estão disponíveis abaixo e isolados por modalidade.
                      </p>
                      <p style={{ margin: 0, fontSize: '11px', color: 'var(--text3)', fontStyle: 'italic' }}>
                        Para análise completa desta modalidade, utilize o checklist de Licenciamento Comercial como referência enquanto o módulo específico é implementado.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ padding: '16px', background: 'rgba(0,0,0,0.02)', border: '1px solid var(--border)', borderRadius: '6px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', fontSize: '12px' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '150px 1fr', gap: '8px', alignItems: 'center' }}>
                      <span>Zona KMZ:</span>
                      <select 
                        value={checklistData[currentProjKey]?.zona_kmz || ''} 
                        onChange={e => setChecklistData({
                          ...checklistData,
                          [currentProjKey]: { ...checklistData[currentProjKey], zona_kmz: e.target.value }
                        })}
                        style={{ padding: '4px 8px', fontSize: '12px', borderRadius: '4px', border: '1px solid var(--border)', background: 'var(--card-bg)', color: 'var(--text1)' }}
                      >
                        <option value="">Selecione...</option>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '150px 1fr', gap: '8px', alignItems: 'center' }}>
                      ...
                      <input 
                        type="number" 
                        value={checklistData[currentProjKey]?.num_pavimentos || ''}
                        onChange={e => setChecklistData({
                          ...checklistData,
                          [currentProjKey]: { ...checklistData[currentProjKey], num_pavimentos: e.target.value }
                        })}
                        style={{ padding: '4px 8px', fontSize: '12px', borderRadius: '4px', border: '1px solid var(--border)', background: 'var(--card-bg)', color: 'var(--text1)' }}
                      />
                    </div>
                ...
                {/* Análise de Afastamentos — já funcional via currentProjKey */}
                <CardAfastamentos
                  zonaKey={checklistData[currentProjKey]?.zona_kmz || ''}
                  numPavimentos={checklistData[currentProjKey]?.num_pavimentos || ''}
                  nsapl={checklistData[currentProjKey]?.afast_nsapl || false}
      ...
      {isEditMovDateOpen && (
        <Modal
          title={`✏️ Corrigir Data do Histórico: ${selectedMovDate?.label}`}
          onClose={() => setIsEditMovDateOpen(false)}
          width="400px"
          footer={<>
            <button className="btn btn-outline" onClick={() => setIsEditMovDateOpen(false)}>Cancelar</button>
            <button className="btn btn-primary" onClick={handleUpdateMovDate} disabled={movDateLoading}>
              {movDateLoading ? 'Salvando...' : '💾 Salvar Nova Data'}
            </button>
          </>}
        >
          <div className="fg">
            <label>Nova Data e Hora (Retroativa)</label>
            <input 
              type="datetime-local" 
              value={newMovDateVal}
              onChange={e => setNewMovDateVal(e.target.value)}
              className="input"
            />
          </div>
          {movDateAlert && <div className="alert alert-err" style={{marginTop: 10}}>{movDateAlert}</div>}
        </Modal>
      )}
